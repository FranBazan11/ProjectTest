...
//
//  Created by Juan Francisco Bazan Carrizo on 14/03/2022.
//

import Foundation
import UIKit

final class TimelineStepper: UIView {
    ...
    private func drawStepes(totalDelay: Double,
        ...
        for item in 1 ... 3 {
            if item == 2 {
                ...
                self.contentView.addArrangedSubview(self.createCurrentStepCircle())
                ...
                self.contentView.addArrangedSubview(self.createStepCircle())
                totaldelayAnimation = totaldelayAnimation + delay
                ...
                self.contentView.addArrangedSubview(self.createNextStepCircle())
                totaldelayAnimation = totaldelayAnimation + delay
            }
        }
    }
...
//            color = UIColor(red: 222, green: 222, blue: 222, alpha: 1)
        ...
        self.contentView.addArrangedSubview(self.drawLine(color: color, animation: animation, delay: totaldelayAnimation))
    }
    // MARK: - Helper
...
//        let shapeLayer2 = self.createShapeLayer(strokeColor: UIColor(red: 0, green: 166, blue: 80, alpha: 1), lineWidth: 2)
    ...
    private func createCurrentStepCircle(animation: Bool = false) -> UIView {
        ...
        let haloCircleView = createBaseView(size: sizeView, background: UIColor.clear)
